...
Keys: `q` quit, `v` view, `r` raw, `s` split, `e` edit, `Right`/`l` next tab, `Left`/`h` previous tab, `x`/`d` discard current tab, `PgUp`/`Ctrl+U` page up, `PgDn`/`Ctrl+D` page down, `?` toggle help, `Esc` close help.
